...
export default function CreateInviteClientShell() {
  ...
  async function onCreateInvite() {
    ...
    try {
      ...
      if (data?.error === 'no_invite_credits') {
        setCredits(0)
        setError('You have no invite credits remaining.')
        return
      }
  ...
  function onShare() {
    if (!inviteUrl) return
    if (navigator.share) {
      void navigator.share({ title: 'Join me on BetterMate', text: 'I saved a spot for you on BetterMate — invite only, values-first.', url: inviteUrl })
    } else { void onCopy() }
  }
        ...
        {!inviteUrl ? (
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <h1 style={{ margin: 0, color: gold, fontSize: 20, fontWeight: 700 }}>Your invite is ready</h1>
              {credits !== null && (
                <span style={{ background: 'rgba(201,169,110,0.12)', border: '1px solid rgba(201,169,110,0.3)', borderRadius: 8, padding: '4px 10px', fontSize: 11, color: gold, fontFamily: 'Georgia, serif' }}>
                  {credits} left
                </span>
              )}
            </div>
